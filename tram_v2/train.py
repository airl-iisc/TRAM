"""Train TRAM V2 (V1 + CBAM).

Adds CBAM on top of V1, plus a cosine LR schedule, mixed precision, and early
stopping — matching the configuration that produced the paper numbers (which
converged in 44 epochs instead of 100).
...
from final.tram_v2.model import build_model


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train TRAM V2 on SSS_OD")
    ...
    p.add_argument("--output-dir", default="runs/tram_v2")
...
def main() -> None:
    ...
    ckpt_path = os.path.join(args.output_dir, "tram_v2_best.pth")
